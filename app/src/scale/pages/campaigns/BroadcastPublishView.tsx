...
import { BroadcastSendingProgressPanel } from "@/scale/components/BroadcastSendingProgressPanel";
import { BroadcastStatusBadge } from "@/scale/components/BroadcastStatusBadge";
import { scaleAudienceDetailHref, broadcastDetailHref } from "@/scale/lib/paths";
...
import { scaleApi, ScaleApiError, type BroadcastDispatchProgress } from "@/lib/scale/api";
...
export function BroadcastPublishView() {
  ...
  async function handleConfirmSend() {
    ...
    try {
      ...
      const result = await scaleApi.sendBroadcast(broadcastId);
  ...
  async function handleTestSend() {
    ...
    try {
      ...
      if (!saved) return;
      if (!(await prepareSendCredentials())) return;
      await scaleApi.testSendBroadcast(broadcastId, testEmail);
  ...
  async function handleSchedule() {
    ...
    try {
      ...
      if (!saved) return;
      if (!(await prepareSendCredentials())) return;
      const updated = await scaleApi.scheduleBroadcast(broadcastId, new Date(scheduleAt).toISOString());
      setBroadcast(updated);
  ...
  async function handleCancelSchedule() {
    try {
      const updated = await scaleApi.cancelSchedule(broadcastId);
      setBroadcast(updated);
                ...
                {broadcast.audienceGroupId ? (
                  <>
                    ...
                    <Button
                      ...
                      nativeButton={false}
                      render={
                        <Link href={scaleAudienceDetailHref(broadcast.audienceGroupId)} />
                      }
                    >
