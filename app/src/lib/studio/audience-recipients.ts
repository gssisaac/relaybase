...
/** Unique recipients across all Studio audience groups. */
export async function fetchAllAudienceRecipients(): Promise<AudienceRecipient[]> {
  const { groups } = await studioAudienceApi.listGroups();
  const byEmail = new Map<string, AudienceRecipient>();

  for (const group of groups) {
    const detail = await studioAudienceApi.getGroup(group.id);
