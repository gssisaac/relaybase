...
export function CampaignSentOverviewView() {
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <div className="min-w-0 space-y-2">
          ...
          <CampaignsSectionNav active="sent" />
        </div>
      </DesktopTitleBar>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-auto">
        <div className={dashboardScrollBodyClassName("space-y-4")}>
          ...
          {!data ? (
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">No sent campaigns yet</CardTitle>
                <CardDescription>
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Finished sends, newest first. Open a row for per-campaign stats.</CardDescription>
                </CardHeader>
                <CardContent className="divide-y divide-border p-0">
                  {data.campaigns.map((row) => (
                    <Link
                      key={row.id}
                      href={campaignDetailHref(row.id, "stats")}
              ...
              {data.topLinks.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Tracked clicks across finished campaigns.</CardDescription>
                  </CardHeader>
                  <CardContent className="divide-y divide-border p-0">
                    {data.topLinks.map((row) => (
                      <div
                        key={`${row.campaignId}:${row.url}`}
                        className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm"
                      >
                        <a
                          href={row.url}
