import type {
  AutomationSend,
  AutomationStats,
  AutomationTrackingEvent,
} from "../../db/types";

export function emptyAutomationStats(): AutomationStats {
  return {
    triggered: 0,
    matched: 0,
    deduped: 0,
...
export function normalizeAutomationStats(raw: Partial<AutomationStats> | undefined): AutomationStats {
  const base = emptyAutomationStats();
  if (!raw) return base;
  return {
...
export function rollupAutomationStatsFromSends(
  sends: AutomationSend[],
  trackingEvents: AutomationTrackingEvent[] = [],
): Omit<AutomationStats, "triggered" | "matched" | "deduped"> {
  ...
  const complained = new Set(
    trackingEvents.filter((e) => e.type === "complaint").map((e) => e.automationSendId),
  ).size;
  return {
    sent,
    delivered,
