...
export function syncAccountComplianceMirror(draft: StudioDataStore): void {
...
export function ensureComplianceIdentitiesFromLegacy(draft: StudioDataStore, now: string): void {
  ...
  for (const row of draft.newsletters) {
