...
export const scaleTriggers = new Hono();
...
scaleTriggers.get("/stats", (c) => {
  return c.json(buildTriggerStatsOverview());
});

scaleTriggers.post("/", async (c) => {
  let body: {
    name?: string;
    domain?: string;
...
scaleTriggers.post("/fire/internal/:event", async (c) => {
...
scaleTriggers.post("/:id/activate", (c) => {
  const id = c.req.param("id")!;
  const existing = findTrigger(id);
  if (!existing) return c.json({ error: "not found" }, 404);
...
scaleTriggers.post("/:id/rotate-webhook-secret", (c) => {
  const id = c.req.param("id")!;
  const existing = findTrigger(id);
  if (!existing) return c.json({ error: "not found" }, 404);
...
scaleTriggers.post("/:id/test-send", async (c) => {
  const id = c.req.param("id")!;
  const existing = findTrigger(id);
  if (!existing) return c.json({ error: "not found" }, 404);
...
scaleTriggers.get("/:id/activity", (c) => {
  const id = c.req.param("id")!;
