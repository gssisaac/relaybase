...
|------|-------------|-----------|----------------------------------|---------------------------|
| **N-01** | **OS 보안 저장소 (Keyring / Keychain)** | `auth/keyring_store.rs` | **이유**: Passtoken·owner/team refresh·CF OAuth refresh를 OS 보안 저장소에 암호화 저장.<br>**유지**: Keyring get/set/delete 프리미티브만. | **처리**: Owner access JWT → `app/src/lib/desktop/auth/owner-session.ts` **JS 메모리** (`mail`/`console` scope). Owner refresh(mail·console) + Worker URL → 탭 **`sessionStorage` `relaybase:owner-session`** (`auth/web-owner-persist.ts`) — 같은 탭 하드 새로고침 시 `restoreWebOwnerSession()`이 `/console/refresh`로 access 재발급, 탭을 닫으면 소멸. Passtoken은 **저장하지 않음** — `/login`에서 입력(웹 설치 직후 1회 핸드오프) 후 `webOwnerLogin()` (`web-owner-bridge.ts`, `hasPasstoken: false`). Owner 토큰은 localStorage·쿠키 금지(BFF 쿠키 없음, M-05: 브라우저 `fetch` + Bearer). 웹 미인증 랜딩은 **`/login`**, 설치는 `/setup`. CF OAuth install → 서버 **sealed httpOnly 쿠키** `rb_cf_oauth` (`server/cloudflare/session.ts`, `/api/oauth/*`). Worker 연결 메타 → `sessionStorage` `relaybase.web.credentials` (`web-credentials.ts`). Team 비밀번호 → `sessionStorage` `relaybase:email-session` + 메모리 (`email-session.ts`). API 키 평문 → `localStorage` `relaybase:api-keys-vault:v1` (`bridge/storage.ts`).<br>**상태**: ✅ 동작. ⚠️ 웹은 owner refresh·team 비밀번호를 OS급 비밀이 아닌 **탭 `sessionStorage`**에 두므로(passtoken은 미저장) 데스크톱과 보안·지속성 모델이 다름. |
| **N-02** | **OS 하드웨어 생체인증 (Touch ID / Windows Hello)** | `auth/touch_id.rs`<br>`tauri-plugin-biometry` | **이유**: 키체인 읽기 전 `LocalAuthentication` 게이트.<br>**유지**: 생체 프롬프트 및 성공/실패만 반환. | **처리**: `desktopOwnerTouchId` / `desktopOwnerLoginFromKeyring`는 웹에서 **throw** (`bridge/owner.ts`). Owner 대시보드는 `DesktopDashboardGate` → `WebOwnerGate` + `hasWebOwnerSession()`으로 **AppSessionStore·Touch ID 단계 생략** (`_shell/DesktopDashboardGate.tsx`). Console 잠금 해제는 메모리 access → 없으면 탭 `sessionStorage` refresh로 복원 후 `ensureAccessToken('console')` (`web-owner-bridge.ts`).<br>**상태**: ✅ 의도적 생략(대체: `/login` passtoken 입력·같은 탭 세션 복원). 생체 UI는 데스크톱 전용. |
