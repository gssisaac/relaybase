import {
  newsletterService,
  studioDocumentService,
  subscriberGroupService,
} from "@services/index";

import { Hono } from "hono";
export const studioUnsubscribe = new Hono();

function newsletterListLabel(broadcastId: string): string {
  const broadcast = studioDocumentService.read().newsletters.find((row) => row.id === broadcastId);
  if (!broadcast) return "this list";
  const subject = newsletterService.subject(studioDocumentService.read(), broadcast).trim();
  return subject || "this list";
}

// GET — confirmation only (no side effects; avoids prefetch auto-unsubscribe)
studioUnsubscribe.get("/:broadcastId/:token", (c) => {
  const { broadcastId, token } = c.req.param();
  const { broadcast, contact } = subscriberGroupService.lookupUnsubscribeContact(broadcastId, token);

  if (!contact) {
    c.header("Content-Type", "text/html; charset=utf-8");
    return c.body(
      subscriberGroupService.unsubscribeHtmlPage({
        heading: "Invalid or expired unsubscribe link",
        subtext:
          "If you continue to receive unwanted emails, please contact the sender directly.",
      }),
      404,
    );
  }

  if (contact.sendStatus === "unsubscribed") {
    c.header("Content-Type", "text/html; charset=utf-8");
    return c.body(
      subscriberGroupService.unsubscribeHtmlPage({
        heading: "Already unsubscribed",
        subtext: `${contact.email} is not on '${newsletterListLabel(broadcastId)}'.`,
      }),
    );
  }

  c.header("Content-Type", "text/html; charset=utf-8");
  return c.body(
    subscriberGroupService.unsubscribeHtmlPage({
      heading: "Unsubscribe?",
      subtext: `${contact.email} will stop receiving '${newsletterListLabel(broadcastId)}'.`,
      formAction: subscriberGroupService.unsubscribePath(broadcastId, token),
      footer: `<footer><a class="link" href="${subscriberGroupService.unsubscribePath(broadcastId, token)}/resubscribe">Unsubscribed by mistake?</a></footer>`,
    }),
  );
});

// POST — RFC 8058 one-click + form confirm
studioUnsubscribe.post("/:broadcastId/:token", async (c) => {
  const { broadcastId, token } = c.req.param();
  const result = subscriberGroupService.performUnsubscribe(broadcastId, token);

  if (!result.ok) {
    return c.json({ error: "invalid token" }, 404);
  }

  const acceptsHtml = c.req.header("Accept")?.includes("text/html");
  if (acceptsHtml) {
    c.header("Content-Type", "text/html; charset=utf-8");
    return c.body(
      subscriberGroupService.unsubscribeHtmlPage({
        heading: "You have been unsubscribed",
        subtext: `${result.email} will no longer receive emails from '${result.listName}'.`,
        footer: `<a class="link" href="${subscriberGroupService.unsubscribePath(broadcastId, token)}/resubscribe">Unsubscribed by mistake? Click here to resubscribe.</a>`,
      }),
    );
  }

  return c.body(null, 200);
});

studioUnsubscribe.get("/:broadcastId/:token/resubscribe", (c) => {
  const { broadcastId, token } = c.req.param();
  const { broadcast, contact } = subscriberGroupService.lookupUnsubscribeContact(broadcastId, token);

  if (!contact) {
    c.header("Content-Type", "text/html; charset=utf-8");
    return c.body(
      subscriberGroupService.unsubscribeHtmlPage({
        heading: "Invalid or expired unsubscribe link",
        subtext:
          "If you continue to receive unwanted emails, please contact the sender directly.",
      }),
      404,
    );
  }

  subscriberGroupService.resubscribe(broadcastId, token);

  c.header("Content-Type", "text/html; charset=utf-8");
  return c.body(
    subscriberGroupService.unsubscribeHtmlPage({
      heading: "You're resubscribed",
      subtext: `${contact.email} will receive emails from '${newsletterListLabel(broadcastId)}' again.`,
    }),
  );
});
