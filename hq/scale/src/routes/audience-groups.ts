...
export const scaleAudience = new Hono();

// GET /scale/audience-groups
scaleAudience.get("/", (c) => {
  const groups = store
    .read()
...
// POST /scale/audience-groups/test
scaleAudience.post("/test", async (c) => {
  let body: {
    endpointUrl?: string;
    credential?: string;
...
// POST /scale/audience-groups
scaleAudience.post("/", async (c) => {
  let body: {
    name?: string;
    domain?: string;
    workerUrl?: string;
...
// GET /scale/audience-groups/:id
scaleAudience.get("/:id", (c) => {
...
// PATCH /scale/audience-groups/:id
scaleAudience.patch("/:id", async (c) => {
...
// DELETE /scale/audience-groups/:id
scaleAudience.delete("/:id", (c) => {
  const id = c.req.param("id");
...
// POST /scale/audience-groups/:id/contacts
scaleAudience.post("/:id/contacts", async (c) => {
  const group = findAudienceGroup(c.req.param("id"));
  if (!group) return c.json({ error: "not found" }, 404);
...
// DELETE /scale/audience-groups/:id/contacts?contactId=
scaleAudience.delete("/:id/contacts", (c) => {
  const group = findAudienceGroup(c.req.param("id"));
  if (!group) return c.json({ error: "not found" }, 404);
...
// PATCH /scale/audience-groups/:id/contacts/:contactId { sendStatus: "active" | "unsubscribed" }
scaleAudience.patch("/:id/contacts/:contactId", async (c) => {
  const group = findAudienceGroup(c.req.param("id"));
  if (!group) return c.json({ error: "not found" }, 404);
