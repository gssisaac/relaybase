...
import { scaleBroadcastAudience } from "./broadcast-audience";

export const scaleBroadcasts = new Hono();
...
scaleBroadcasts.get("/sent-stats", (c) => {
...
scaleBroadcasts.get("/in-progress", (c) => {
  const data = store.read();
...
scaleBroadcasts.route("/:broadcastId/audience", scaleBroadcastAudience);

// GET /scale/broadcasts/:id
scaleBroadcasts.get("/:id", (c) => {
  const row = findBroadcast(c.req.param("id")!);
  if (!row) return c.json({ error: "not found" }, 404);
  return c.json(serializeBroadcast(row));
});

// PATCH /scale/broadcasts/:id
scaleBroadcasts.patch("/:id", async (c) => {
...
scaleBroadcasts.post("/:id/test-send", async (c) => {
...
scaleBroadcasts.post("/:id/send", async (c) => {
  const id = c.req.param("id")!;
  const existing = findBroadcast(id);
  if (!existing) return c.json({ error: "not found" }, 404);
  if (existing.status !== "draft") {
...
scaleBroadcasts.post("/:id/schedule", async (c) => {
  const id = c.req.param("id")!;
...
scaleBroadcasts.post("/:id/cancel-schedule", async (c) => {
  const id = c.req.param("id")!;
  const broadcast = findBroadcast(id);
  if (!broadcast) return c.json({ error: "not found" }, 404);
...
scaleBroadcasts.post("/:id/duplicate", (c) => {
...
scaleBroadcasts.get("/:id/stats", (c) => {
  const id = c.req.param("id")!;
  const broadcast = findBroadcast(id);
  if (!broadcast) return c.json({ error: "not found" }, 404);
