...
export const crmAutomations = new Hono();
...
crmAutomations.post("/fire/internal/:event", async (c) => {
...
crmAutomations.post("/:id/activate", (c) => {
  const id = c.req.param("id")!;
  const existing = findAutomation(id);
  if (!existing) return c.json({ error: "not found" }, 404);
...
crmAutomations.post("/:id/pause", (c) => {
  const id = c.req.param("id")!;
  const existing = findAutomation(id);
  if (!existing) return c.json({ error: "not found" }, 404);
...
crmAutomations.post("/:id/rotate-webhook-secret", (c) => {
  const id = c.req.param("id")!;
  const existing = findAutomation(id);
  if (!existing) return c.json({ error: "not found" }, 404);
...
crmAutomations.post("/:id/test-send", async (c) => {
  const id = c.req.param("id")!;
  const existing = findAutomation(id);
  if (!existing) return c.json({ error: "not found" }, 404);
  ...
  const payload: Record<string, unknown> = {
    ...
    message: "This is a test message from CRM automations.",
    ...(body.payload ?? {}),
  };
...
crmAutomations.get("/:id/activity", (c) => {
  const id = c.req.param("id")!;
