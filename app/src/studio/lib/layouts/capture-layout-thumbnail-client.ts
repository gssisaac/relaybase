...
import { renderLayoutPreviewHtml } from "@/studio/lib/layouts/render-layout-preview-html";
...
function wrapFragmentHtml(bodyHtml: string): string {
  ...
  .layout-thumb-body { max-width: 600px; margin: 0 auto; font-family: sans-serif; }
  .layout-thumb-body p { margin: 0.75em 0; }
  .layout-thumb-body p:first-child { margin-top: 0; }
  .layout-thumb-body p:last-child { margin-bottom: 0; }
  ${GMAIL_LINK_STYLES}
</style></head><body><div class="layout-thumb-body">${bodyHtml}</div></body></html>`;
}
...
export async function captureLayoutThumbnailBlob(input: {
  layoutId: string;
  htmlSource: string;
  variablesSchema: TemplateVariablesSchema | null | undefined;
}): Promise<Blob> {
  const plainText = isPlainTextTemplate(input.layoutId);
  ...
  const srcdoc = plainText
    ? wrapPlainTextDocument(bodyHtml)
    : /<\s*html[\s>]/i.test(bodyHtml)
      ? bodyHtml
      : wrapFragmentHtml(bodyHtml);
