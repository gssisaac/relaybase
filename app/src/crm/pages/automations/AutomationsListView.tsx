...
import { automationStatsLine, automationTriggerSummary } from "@/crm/lib/automation-trigger-label";
import { automationDetailHref } from "@/crm/lib/paths";
...
export function AutomationsListView() {
  ...
  const load = useCallback(async (force?: boolean) => {
    ...
    try {
      const list = await crmApi.listAutomations();
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    try {
      const created = await crmApi.createAutomation({ name, domain, purpose: newPurpose });
      toast.success(`Automation '${created.name}' created`);
      setCreateOpen(false);
      resetCreate();
      router.push(automationDetailHref(created.id, "content", created.status));
    } catch (e) {
      toast.error(e instanceof CrmApiError ? e.message : "Could not create automation");
      setCreating(false);
    }
  }
