...
export const scaleNewsletterAudience = new Hono();

// GET /scale/newsletters/:newsletterId/audience?status=&q=
scaleNewsletterAudience.get("/", (c) => {
...
// POST /scale/newsletters/:newsletterId/audience/sync — live group; returns current counts
scaleNewsletterAudience.post("/sync", async (c) => {
  const newsletterId = c.req.param("newsletterId")!;
