...
export function TriggerSourceSection() {
  ...
  const webhookUrl = useMemo(() => {
    if (!trigger) return "";
    return `${getScaleApiBase()}/scale/hooks/trigger/${trigger.id}`;
  }, [trigger]);
  ...
  async function saveTrigger() {
    ...
    try {
      ...
      let updated = await scaleApi.updateTrigger(triggerId, {
        source: patch,
        cooldownSeconds,
        applyMarketingSuppression: applySuppression,
      });
      if (patch.type === "http_webhook" && patch.secret === "pending-rotate") {
        const rotated = await scaleApi.rotateTriggerWebhookSecret(triggerId);
  ...
  async function handleActivate() {
    ...
    try {
      await saveTrigger();
      const updated = await scaleApi.activateTrigger(triggerId);
      setTrigger(updated);
  ...
  async function handlePause() {
    try {
      const updated = await scaleApi.pauseTrigger(triggerId);
      setTrigger(updated);
  ...
  async function rotateSecret() {
    try {
      const res = await scaleApi.rotateTriggerWebhookSecret(triggerId);
  ...
  async function sendTest() {
    ...
    setTestSending(true);
    try {
      await scaleApi.testSendTrigger(triggerId, { email, name: "Test User" });
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                <code className="rounded bg-muted px-1">
                  POST /scale/triggers/fire/internal/{internalEvent}
                </code>{" "}
                (Scale API auth).
              </p>
            </div>
