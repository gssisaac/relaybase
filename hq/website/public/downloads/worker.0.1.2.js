...
// src/lib/cf-account-id.ts
function normalizeCfAccountId(raw2) {
  const id = raw2?.trim() ?? "";
  if (!/^[a-f0-9]{32}$/i.test(id)) return null;
  return id.toLowerCase();
}
var init_cf_account_id = __esm({
  "src/lib/cf-account-id.ts"() {
    "use strict";
    __name(normalizeCfAccountId, "normalizeCfAccountId");
  }
});
...
async function setOwnerCfAccountId(db, cfAccountId) {
  if (!db) return;
  const id = cfAccountId.trim();
  if (!id) return;
  await db.insert(ownerConfig).values({ id: 1, cfAccountId: id }).onConflictDoUpdate({
    target: ownerConfig.id,
    set: { cfAccountId: id }
  }).run();
}
var init_owner = __esm({
  "db/app/owner.ts"() {
    ...
    __name(setOwnerCfAccountId, "setOwnerCfAccountId");
  }
});
...
function messageLooksLikePlanError(message) {
  ...
  if (lower.includes("[10105]") || lower.includes("not_entitled") || lower.includes("not entitled") || lower.includes("workers paid") || lower.includes("paid plan")) {
    return true;
  }
  if (lower.includes("[2036]") && lower.includes("unauthorized") && lower.includes("email/sending")) {
    return true;
  }
  return false;
}
...
function cloudflareSendingErrorHint(errors) {
  ...
  if (code === 2036 || msg.toLowerCase().includes("unauthorized") && msg.includes("[2036]")) {
...
function zonesOnPinnedAccount(zones, pinnedAccountId) {
...
function zoneBelongsToPinnedAccount(zoneAccountId, pinnedAccountId) {
  const pinned = normalizeCfAccountId(pinnedAccountId);
  if (!pinned) return false;
  return normalizeCfAccountId(zoneAccountId) === pinned;
}
function zonesListQuery(page, pinnedAccountId) {
  const pinned = normalizeCfAccountId(pinnedAccountId);
  if (!pinned) {
...
var init_cloudflare_zones = __esm({
  "src/lib/cloudflare-zones.ts"() {
    "use strict";
    init_cf_account_id();
    __name(mapCfZoneRow, "mapCfZoneRow");
    __name(zonesOnPinnedAccount, "zonesOnPinnedAccount");
    __name(zoneBelongsToPinnedAccount, "zoneBelongsToPinnedAccount");
    __name(zonesListQuery, "zonesListQuery");
  }
});
...
function encodeBase64Chunked(buffer) {
...
function buildMimeMessage(params) {
  ...
  const parts = [
    `--${mixedBoundary}`,
    `Content-Type: ${bodyPart.contentType}`,
    ...params.html?.trim() ? [""] : ["Content-Transfer-Encoding: 7bit", ""],
    bodyPart.body
  ];
  for (const attachment of attachments) {
    const encoded = encodeBase64Chunked(attachment.content);
    const wrapped = encoded.replace(/.{1,76}/g, (line) => `${line}\r
`).trim();
    parts.push(
      `--${mixedBoundary}`,
...
var init_mime = __esm({
  "src/lib/mime.ts"() {
    ...
    __name(encodeBase64Chunked, "encodeBase64Chunked");
    ...
    CloudflareClient = class {
      ...
      async sendEmail(params) {
        ...
        const needsRaw = Boolean(
          fromName || params.inReplyTo?.trim() || params.references?.trim() || (params.rawAttachments?.length ?? 0) > 0
        );
        if (needsRaw) {
      ...
      async listZones() {
        ...
        let page = 1;
        for (; ; ) {
          const data = await this.request(`/zones?${zonesListQuery(page, pinned)}`);
      ...
      async resolveZoneId(domain) {
        ...
        const data = await this.request(`/zones?${params.toString()}`);
...
// src/lib/pinned-cf-account.ts
var pinned_cf_account_exports = {};
__export(pinned_cf_account_exports, {
  pinnedCfAccountId: () => pinnedCfAccountId
});
async function pinnedCfAccountId(env) {
...
var init_pinned_cf_account = __esm({
  "src/lib/pinned-cf-account.ts"() {
    "use strict";
    init_app();
    init_owner();
    init_cf_account_id();
    __name(pinnedCfAccountId, "pinnedCfAccountId");
  }
});
...
async function createCloudflareClient(env, opts) {
  ...
  if (!accountId) {
    const { pinnedCfAccountId: pinnedCfAccountId2 } = await Promise.resolve().then(() => (init_pinned_cf_account(), pinned_cf_account_exports));
    accountId = await pinnedCfAccountId2(env);
  }
...
// src/lib/cloudflare-account.ts
init_cf_account_id();
var CF_API = "https://api.cloudflare.com/client/v4";
async function resolveCfAccountIdFromToken(token) {
  ...
  try {
    ...
    const ids = [
      ...new Set(
        data.result.map((row) => normalizeCfAccountId(row.id)).filter((id) => Boolean(id))
      )
    ];
...
__name(resolveCfAccountIdFromToken, "resolveCfAccountIdFromToken");
...
async function resetOwner(env, input) {
  ...
  if (!expectedAccount) {
    return {
      error: "Could not verify which Cloudflare account authorized this token. Try Authorize again.",
      status: 401
    };
  }
...
async function discoverRecoverAccount(token) {
  const bearer = token.trim();
  if (!bearer) return null;
  try {
    const res = await fetch(
...
__name(discoverRecoverAccount, "discoverRecoverAccount");
...
async function requireSchemaAuth(c, _hasOwner) {
...
__name(decodeBase64String, "decodeBase64String");
var BASE64_ASCII_PREFIXES = [
  "iVBORw0KGgo",
  // PNG
  "/9j/",
  // JPEG
...
function isAsciiBase64Payload(bytes) {
  ...
  for (let i = 0; i < sampleLen; i++) {
    const c = bytes[i];
    const ok = c >= 65 && c <= 90 || c >= 97 && c <= 122 || c >= 48 && c <= 57 || c === 43 || c === 47 || c === 61 || c === 10 || c === 13;
    if (!ok) return false;
  }
  return sampleLen >= 8;
}
__name(isAsciiBase64Payload, "isAsciiBase64Payload");
function normalizeAttachmentBytes(content) {
...
__name(normalizeAttachmentBytes, "normalizeAttachmentBytes");
...
async function storeSentMail(bucket, params, mailDb) {
  ...
  const attachmentMeta = [];
  for (let index2 = 0; index2 < (params.attachments?.length ?? 0); index2++) {
    const attachment = params.attachments[index2];
    const attachmentId = String(index2);
    await bucket.put(
...
async function getMailAttachment(bucket, kind, params) {
  const domain = params.domain.trim().toLowerCase();
...
__name(getMailAttachment, "getMailAttachment");
...
async function probeCfApiTokenValid(token) {
  try {
    const res = await fetch(`${CF_API2}/zones?per_page=1`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    const data = await res.json();
...
`;
var MAIL_0001 = `-- Unified mailbox D1: list/count/cursor table for inbound + sent, plus a
...
function planRequiredSendingHealthDomain(domain, zoneId, probeMessage, cloudflareSendingUrl) {
...
__name(planRequiredSendingHealthDomain, "planRequiredSendingHealthDomain");
...
consoleSendingOnboard.post("/", async (c) => {
  ...
  const bodyAccountId = typeof body.accountId === "string" ? body.accountId : "";
  let cf;
  try {
  ...
  try {
    ...
    if (isCloudflarePlanError(message)) {
      ...
      return c.json(
        {
          error: body2.error,
          code: body2.code ?? "cf_workers_paid_required",
          domain
        },
        403
      );
...
consoleZones.get("/", async (c) => {
  ...
  try {
    ...
    return c.json(
      {
        error: error instanceof Error ? error.message : "Cloudflare API is not configured on this worker \u2014 add a CF_API_TOKEN secret (Email Routing + Zone Read + DNS) so the Worker can manage domains and DNS"
      },
      503
    );
  }
...
// src/lib/mail/send-attachments.ts
var MAX_SEND_MESSAGE_BYTES = 5 * 1024 * 1024;
...
async function resolveSendAttachments(bucket, items) {
  ...
  for (const item of items) {
    ...
    if (item.contentBase64?.trim()) {
      content = decodeBase642(item.contentBase64.trim());
    } else if (item.source) {
...
__name(resolveSendAttachments, "resolveSendAttachments");
function assertSendMessageSize(text2, html, attachments) {
  const bodyBytes = new TextEncoder().encode(text2).byteLength;
  const htmlBytes = html?.trim() ? new TextEncoder().encode(html).byteLength : 0;
  const attachmentBytes2 = attachments.reduce(
    (sum, item) => sum + item.size,
    0
  );
  const estimated = bodyBytes + htmlBytes + attachmentBytes2;
...
__name(assertSendMessageSize, "assertSendMessageSize");
...
function attachmentsForEmailBinding(attachments) {
...
__name(attachmentsForEmailBinding, "attachmentsForEmailBinding");
...
async function sendMailMessage(env, body, source, options) {
  ...
  try {
    ...
    try {
      ...
      assertSendMessageSize(text2, body.html, resolvedAttachments);
    } catch (error) {
    ...
    const result = await sendOutboundEmail(env, {
      ...
      attachments: bindingAttachments.length ? bindingAttachments : void 0,
...
mailSendingHealth.get("/", async (c) => {
  ...
  const snapshot = await collectSendingHealth(mailbox.domains, cf, {
    accountId: normalizeCfAccountId(c.req.query("accountId")) ?? c.env.CF_ACCOUNT_ID,
    probeError
  });
