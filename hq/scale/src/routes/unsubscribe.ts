...
export const scaleUnsubscribe = new Hono();

// GET — confirmation only (no side effects; avoids prefetch auto-unsubscribe)
scaleUnsubscribe.get("/:broadcastId/:token", (c) => {
...
scaleUnsubscribe.post("/:broadcastId/:token", async (c) => {
  const { broadcastId, token } = c.req.param();
...
scaleUnsubscribe.get("/:broadcastId/:token/resubscribe", (c) => {
  const { broadcastId, token } = c.req.param();
  const { broadcast, contact } = lookupUnsubscribeContact(broadcastId, token);
