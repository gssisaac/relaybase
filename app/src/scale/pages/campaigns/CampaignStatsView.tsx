...
import {
  ...
  type CampaignLinkClickStat,
  type CampaignRecipient,
  type CampaignTrackingEvent,
} from "@/lib/scale/api";

const EVENT_LABEL: Record<CampaignTrackingEvent["type"], string> = {
  delivered: "Delivered",
  open: "Opened",
  click: "Clicked",
  bounce: "Bounced",
...
function downloadRecipientsCsv(
  ...
  a.href = url;
  a.download = `${campaignName.replace(/[^a-z0-9]+/gi, "-").toLowerCase() || "campaign"}-recipients.csv`;
  a.click();
  URL.revokeObjectURL(url);
}

export function CampaignStatsView() {
  const { campaignId, campaign, setCampaign } = useCampaignDetail();
  const [recipients, setRecipients] = useState<CampaignRecipient[]>([]);
  const [trackingEvents, setTrackingEvents] = useState<CampaignTrackingEvent[]>([]);
  const [linkClicks, setLinkClicks] = useState<CampaignLinkClickStat[]>([]);
  const [dispatch, setDispatch] = useState<CampaignDispatchProgress | null>(null);
  ...
  useEffect(() => {
    ...
    const loadStats = () => {
      scaleApi
        ...
        .then(({ campaign: row, dispatch: progress, recipients: rows, trackingEvents: events, linkClicks: links }) => {
          if (cancelled) return;
          setCampaign(row);
  ...
  }, [campaignId, campaign?.status, setCampaign]);
  ...
  if (!hasSendData && campaign.status === "scheduled") {
    return (
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-3">
            <div>
              <CardTitle className="text-sm">Scheduled Campaign</CardTitle>
              <CardDescription>
                This campaign is scheduled for {formatWhen(campaign.scheduledAt ?? "")}.
  ...
  if (!hasSendData && campaign.status === "draft") {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">No send data yet</CardTitle>
          <CardDescription>
            Stats appear after you send or schedule this campaign. Compose content, then publish to
            your audience.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button size="sm" nativeButton={false} render={<Link href={campaignDetailHref(campaignId, "publish")} />} >
            Go to Publish
          </Button>
        </CardContent>
      </Card>
  ...
  const totalRecipients = recipients.length || campaign.audienceActiveCount || sent;

  return (
    <div className="space-y-4">
      {campaign.status === "sending" && dispatch ? (
        <CampaignSendingProgressPanel dispatch={dispatch} />
      ) : null}
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {campaign.status === "sending"
              ? `Sending in progress since ${formatWhen(campaign.sentAt ?? "")} · ${processedCount} of ${totalRecipients} processed`
              : campaign.sentAt
                ? `Sent ${formatWhen(campaign.sentAt ?? "")} · ${sent} attempts`
                : "Delivery and engagement for this campaign."}
          </p>
        </div>
        {recipients.length > 0 ? (
          <Button
            size="sm"
            variant="outline"
            onClick={() => downloadRecipientsCsv(campaign.name, recipients)}
          >
