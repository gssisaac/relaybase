# Studio Compliance & Platform Improvements Backlog

**Status:** Active engineering checklist  
**Scope:** `hq/studio` + `app/src/studio` (Audience Group → Broadcast model)  
**Date:** 2026-09-15  

Related: [`studio-audience-broadcast-model.md`](./studio-audience-broadcast-model.md), [`studio-mode-v0.2.md`](./studio-mode-v0.2.md).
...
| Risk | Mitigation |
|------|------------|
| AUP / spam complaint rate on customer Worker or Studio domain | Suppression ledger, rate-limited dispatch, no send without auth |
| Worker subrequest / CPU limits on bulk send | Queue + batch dispatch (scheduler), not single-request fan-out |
| Open/click tracking write spikes on Studio store | Buffered/batched tracking writes in production (same tenant as `hq/studio`) |
| Open redirect abuse on `/studio/t/c` | Allow only `http:` / `https:` targets |
| Trust & Safety phishing via Studio domain | Same redirect guard + monitoring |
...
| 3 | **Click redirect guard** (`/studio/t/c`) — block non-http(s) schemes | Done |
...
| 6 | **Studio API auth** — public only: health, tracking, unsubscribe, assets; mutating routes require secret/session | Done (`STUDIO_API_SECRET`; dev skips if unset). Webhooks: `STUDIO_WEBHOOK_SECRET` |

---

## Nice-to-have (P1+) — after P0 + UI review

Production Studio is intended to run on **the customer’s own Cloudflare stack** (`hq/studio` beside their Worker), not a central Relaybase D1 catalog. Persistence may evolve from dev JSON to a tenant-local store; **no separate “D1 migration” track** is planned here.

| # | Item |
|---|------|
| 7 | Legacy spec banner + canonical `studio-audience-broadcast-model.md` | Done |
...
| 12 | Webhook HMAC for `/studio/webhooks/bounce` |
