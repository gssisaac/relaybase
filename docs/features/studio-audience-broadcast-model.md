# Studio Audience Group & Broadcast Architecture
...
Relaybase Studio targets solo founders and small teams. A separate **Newsletter** entity plus **Subscriber** membership duplicated what **Audience Groups** already express in the product Worker console.
...
## 5. Public endpoints (no Studio session)

| Path | Purpose |
|------|---------|
| `GET/POST /studio/unsubscribe/:broadcastId/:token` | Confirm + RFC 8058 unsubscribe |
| `GET /studio/t/o/...` | Open pixel |
| `GET /studio/t/c/...` | Click redirect (http/https only) |
| `GET /studio/assets/...` | Newsletter images CDN |

All other `/studio/*` routes require API auth in production.
...
- See [`studio-compliance-improvements.md`](./studio-compliance-improvements.md) for P0/P1 checklist.
...
| Route | Purpose |
|-------|---------|
| `/studio/audience` | Groups, contacts, settings, data source |
| `/studio/broadcasts` | Broadcast list + detail (content, publish, stats, settings) |
