...
export const crmAutomationHooks = new Hono();
...
// POST /crm/hooks/automation/:automationId — http_webhook trigger
crmAutomationHooks.post("/automation/:automationId", async (c) => {
...
// POST /crm/hooks/form/:formKey — form_submit trigger
crmAutomationHooks.post("/form/:formKey", async (c) => {
...
// POST /crm/hooks/inbound — mailbox_inbound (Worker → CRM)
crmAutomationHooks.post("/inbound", async (c) => {
  if (!verifyCrmWebhookSecret(c)) {
    return c.json({ error: "unauthorized" }, 401);
  }
