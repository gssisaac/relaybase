...
import { crmApi, CrmApiError, type CrmComplianceIdentity } from "@/lib/crm/api";
...
export function ComplianceIdentityEditor({
  ...
  onIdentitySaved?: (identity: CrmComplianceIdentity) => void;
  compact?: boolean;
  description?: string;
}) {
  const [identities, setIdentities] = useState<CrmComplianceIdentity[]>([]);
  ...
  async function saveIdentity() {
    ...
    setSaving(true);
    try {
      const { identity } = await crmApi.updateComplianceIdentity(editingId, {
      ...
      toast.error(err instanceof CrmApiError ? err.message : "Could not save");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function createIdentity() {
    setCreating(true);
    try {
      const { identity } = await crmApi.createComplianceIdentity({
      ...
      if (mode === "account-default") {
        await crmApi.updateAccountLink({ defaultComplianceIdentityId: identity.id });
      }
      onIdentitySaved?.(identity);
      toast.success("Compliance sender created");
    } catch (err) {
      toast.error(err instanceof CrmApiError ? err.message : "Could not create");
    } finally {
      setCreating(false);
    }
  }
