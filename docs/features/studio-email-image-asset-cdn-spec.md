# Studio Newsletter Image Assets & Cloudflare R2 CDN Specification

**Status:** Approved Architecture Draft  
**Target Engine:** `hq/studio` (Cloudflare Workers + R2 CDN / Local JSON Store)  
**App Editor:** `app/src/lib/markdown-editor/*` & `app/src/studio/pages/newsletters/*`  
**Storage Target:** Cloudflare R2 Bucket `studio-assets` (Public CDN domain)  
**Language:** English  
**Date:** 2026-09-14  
...
To guarantee **100% universal rendering, inbox deliverability, and optimal visual fidelity**, Relaybase Studio establishes the following strict image standards:
...
Public CDN:   https://assets.relaybase.xyz/studio/assets/{newsletterId}/{assetShortId}-{slugifiedName}.{ext}
```
...
- **Execution Flow:**
  ...
  5. POSTs Base64 payload to `/studio/newsletters/:id/assets`.
  6. Server commits asset to storage (local JSON / R2) and returns public CDN URL.
  7. Editor pastes markdown token: `![Summer Launch](https://assets.relaybase.xyz/studio/assets/newsletter_123/H7K2M9P4-summer-launch.jpg)`.
...
- **Execution Flow:**
  ...
  5. HTML template embeds `<img src="https://assets.relaybase.xyz/studio/assets/.../L9X2P1A4-company-logo-dark.png" alt="Company Logo" width="180" style="display:block; border:0; outline:none; text-decoration:none;" />`.
...
- **Execution Flow:**
  ...
  3. Uploads raw GIF to `/studio/newsletters/:id/assets` with `image/gif` MIME type.
  4. Markdown editor inserts `![Feature Demo](https://assets.relaybase.xyz/studio/assets/.../G4K8T2Q1-feature-demo.gif)`.
- **Postcondition:**
...
- **Trigger:** `POST /studio/newsletters/:id/send` or `POST /studio/newsletters/:id/test-send`.
- **Execution Flow:**
  ...
  3. Embeds 1x1 transparent GIF open-tracking pixel (`/studio/t/o/:newsletterId/:memberKey`).
...
- **Execution Flow:**
  1. Gmail's sanitizer scans HTML and replaces image URLs with `https://ci3.googleusercontent.com/proxy/...#https://assets.relaybase.xyz/studio/assets/...`.
  ...
  3. Cloudflare Edge CDN checks cache:
     ...
     - *Cache Miss:* Pulls from R2 bucket `studio-assets`, caches at edge, and returns HTTP 200 with `Content-Type: image/jpeg` and `Cache-Control: public, max-age=31536000, immutable`.
  4. Gmail proxies image to subscriber's viewport.
- **Postcondition:** Image renders instantly; subsequent opens across thousands of subscribers hit Cloudflare CDN cache with zero load on central Studio compute.
...
sequenceDiagram
    ...
    Server-->>Ingest: { url: "https://assets.relaybase.xyz/studio/assets/...", key: "..." }
...
sequenceDiagram
    autonumber
    actor User as Marketer (App UI)
    participant Studio as HQ Studio Server<br/>(relaybase.email)
    ...
    User->>Studio: Click "Send Broadcast Now"<br/>POST /studio/newsletters/:id/send { recipients }
    Note over Studio: Resolve active subscribers<br/>Snapshot newsletter bodyMarkdown & template
    loop For each resolved recipient
        Studio->>Render: renderNewsletterForRecipient({ newsletterId, bodyMarkdown, recipient, ... })
        ...
        Render-->>Studio: Final Rendered HTML Body
        Studio->>Worker: POST {workerUrl}/v1/send<br/>Headers: Authorization: Bearer {apiKey}<br/>Body: { to, subject, html }
        Worker->>SMTP: Dispatch MIME Email (No attachments, pure HTML + text)
        SMTP-->>Worker: Queued / Delivered (250 OK)
        Worker-->>Studio: { ok: true, messageId: "..." }
    end
    Studio->>Studio: Update broadcast status = "sent", record sentAt timestamp
    Studio-->>User: Broadcast complete: { sent: N, failed: 0 }
```
...
sequenceDiagram
    ...
    participant R2 as Cloudflare R2 Bucket<br/>(studio-assets)

    Recipient->>Inbox: Opens Email Message
    Note over Inbox: HTML contains:<br/><img src="https://assets.relaybase.xyz/studio/assets/c123/hero.jpg">
    
    alt Gmail Recipient
        Inbox->>Proxy: Request cached image via GoogleImageProxy
        Proxy->>CDN: GET /studio/assets/c123/hero.jpg<br/>(Unauthenticated HTTPS request)
    else Apple Mail / Desktop Outlook
        Inbox->>CDN: Direct GET /studio/assets/c123/hero.jpg
    end
...
- **Method:** `POST`
- **Path:** `/studio/newsletters/:id/assets`
- **Request Body (JSON):**
  ```json
  {
...
- **Response Body (JSON, 200 OK):**
  ```json
  {
    "url": "https://assets.relaybase.xyz/studio/assets/newsletter_123/H7K2M9P4-summer-launch.jpg",
...
- **Method:** `GET`
- **Path:** `/studio/assets/:newsletterId/:filename`
- **Response Headers:**
  ```http
  HTTP/1.1 200 OK
