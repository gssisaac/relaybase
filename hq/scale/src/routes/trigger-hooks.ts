...
export const scaleTriggerHooks = new Hono();
...
// POST /scale/hooks/trigger/:triggerId — http_webhook
scaleTriggerHooks.post("/trigger/:triggerId", async (c) => {
...
scaleTriggerHooks.post("/form/:formKey", async (c) => {
...
scaleTriggerHooks.post("/inbound", async (c) => {
  if (!verifyCrmWebhookSecret(c)) {
    return c.json({ error: "unauthorized" }, 401);
  }
