...
import { scaleApi, type MessageTemplate } from "@/lib/scale/api";
import {
  EmailListContainer,
  EmailTableHeader,
  EmailTableRow,
...
export function TemplatesListView() {
  ...
  const load = useCallback(async (force?: boolean) => {
    ...
    try {
      const { templates } = await scaleApi.listMessageTemplates();
      setRows(templates);
    } catch {
  ...
  async function createTemplate() {
    ...
    setCreating(true);
    try {
      const { template } = await scaleApi.createMessageTemplate({ name });
      setAddOpen(false);
      setNewName("");
      await load(true);
