 ...
 * Strips syntax while keeping natural paragraph breaks and bullet lists.
 */
...
/** Preview fallback when the editor body is empty. */
export function plainEmailBodyFromMarkdown(
  bodyMarkdown: string,
  emptyFallback = "Nothing to preview yet",
): string {
  const trimmed = bodyMarkdown.trim();
  if (!trimmed) return emptyFallback;
  return markdownToPlainEmailText(trimmed);
}
