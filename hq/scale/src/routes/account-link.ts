...
export const scaleAccountLink = new Hono();

// GET /scale/account-link
scaleAccountLink.get("/", (c) => c.json(serializeAccountLink()));

// PATCH /scale/account-link { domain?, workerUrl?, defaultComplianceIdentityId?, compliance? }
scaleAccountLink.patch("/", async (c) => {
  let body: {
    domain?: string;
    workerUrl?: string;
