# Studio Newsletter, Broadcast & Subscriber Architecture Specification

> **Historical draft — superseded (2026-09-15).**  
> The shipped Studio model **removed the Newsletter / Subscriber entities** in favor of **Audience Group → Broadcast**.  
> **Canonical spec:** [`studio-audience-broadcast-model.md`](./studio-audience-broadcast-model.md)  
> **Compliance backlog:** [`studio-compliance-improvements.md`](./studio-compliance-improvements.md)  
>  
> Keep this file for migration notes and UC copy; do not implement new features against the Newsletter hierarchy below.

**Status:** Superseded  
**Target Engine:** `hq/studio` (Cloudflare Workers + D1 / local JSON store)  
**App UI:** `app/src/studio/*` (Next.js App Router + shadcn/ui)  
**Language:** English  
**Date:** 2026-09-14  
...
In earlier drafts, terminology was conflated: `AudienceGroup` was treated as an account-wide static pool of contacts, `Newsletter` was modeled as a single email send with content and delivery status combined in one record, and dispatches simply unioned all audience groups across the entire account.

This design introduces the canonical, scalable hierarchy:

```text
Account (Workspace / Tenant)
 └── Newsletter (The persistent newsletter/topic/list container; scope of consent)
      ├── Newsletter Settings (Default From, Default Template, Timezone, Fallback Sender)
      ├── Subscriber[] (Newsletter-scoped consent & member records; holds unsubscribe state)
...
```mermaid
erDiagram
    ACCOUNT ||--o{ NEWSLETTER : owns
    NEWSLETTER ||--o{ SUBSCRIBER : holds_consent_for
    NEWSLETTER ||--o{ BROADCAST : schedules
    BROADCAST ||--o{ RECIPIENT : dispatches_to
    SUBSCRIBER ||--o{ RECIPIENT : snapshot_source
    NEWSLETTER ||--o{ NEWSLETTER_ASSET : stores_images
```
...
| **Newsletter** | A long-running publication, newsletter, or marketing stream (e.g., *"Engineering Weekly"*, *"Product Launches"*). It is the **system of record for consent**. | Account | `active` \| `archived` |
| **Subscriber** | A person's explicit opt-in membership **to a specific Newsletter**. Identified by `(newsletterId, email)`. | Newsletter | `subscribed` \| `unsubscribed` \| `pending` \| `bounced` |
| **Source / Audience Source** | An external origin or import pipeline (e.g. JSON endpoint, CSV import, webhook) used to populate Subscribers. | Newsletter / Account | Configuration & sync history |
| **Broadcast** | A single scheduled or immediate delivery event with dedicated content within a Newsletter (e.g., *"Release 2.4 Notes"*, *"Midweek Update"*). | Newsletter | `draft` \| `scheduled` \| `sending` \| `sent` \| `failed` |
...
1. **Newsletter = Consent Scope**: Unsubscribing from *"Product Updates"* flips `Subscriber.status = 'unsubscribed'` on that newsletter only. The user remains `subscribed` in *"Security Advisories"* and retains their Studio Lead/Customer Pipeline status.
...
4. **Separation of Content and Container**: A Newsletter never contains `bodyMarkdown` or `subject`. All copywriting, rich media, and layout templates belong strictly to individual Broadcasts.
...
// ============================================================================
// 1. Newsletters (Persistent Stream & Consent Scope)
// ============================================================================

export type NewsletterStatus = "active" | "archived";

export type NewsletterDataSource = {
  type: "generic_json";
  endpointUrl: string;
  credential?: string;
...
// ============================================================================
// 2. Subscribers (Newsletter-Scoped Membership & Consent)
// ============================================================================
...
// ============================================================================
// 6. Shared Studio & System Support Types
// ============================================================================
...
### 3.1 Newsletter Management Use Cases (`C1` - `C5`)

#### UC-C1: Create New Newsletter
* **Trigger:** User clicks *"New Newsletter"* in the `/studio/newsletters` header toolbar.
...
* **Success Presentation:**
  * **UI/UX:** [Dialog Modal] closes automatically. [Toast] appears: `"Newsletter 'Engineering Updates' created"`.
  * **View Transition:** User is redirected directly to `/studio/newsletters/:id?tab=subscribers`.
* **Failure / Edge Cases:**
  * *Empty Newsletter Name:* [Inline Field] beneath name input: `"Newsletter name is required"`. Save button remains disabled.
  * *Duplicate Slug / Name:* [Inline Field] beneath name/slug input: `"A newsletter with this identifier already exists in this account"`.
  * *Network Failure:* [Toast] (Destructive): `"Failed to create newsletter. Check your connection and retry."`

#### UC-C2: Update Newsletter Settings & Sender Identity
* **Trigger:** User modifies default From Name, From Email, or Default Layout Template in `/studio/newsletters/:id?tab=settings`.
* **System Logic:** Validates email format, saves to `studio_newsletters`. Future broadcasts inherit these defaults automatically.
* **Success Presentation:**
  * **UI/UX:** [Toast]: `"Newsletter settings saved successfully"`. Settings card displays subtle green checkmark icon for 2 seconds.
* **Failure / Edge Cases:**
...
#### UC-C3: Archive Newsletter
* **Trigger:** User selects *"Archive Newsletter"* from the newsletter actions dropdown.
...
* **Success Presentation:**
  * **UI/UX:** [Dialog Modal] confirmation: `"Archive 'Product Weekly'? Pending scheduled broadcasts will be cancelled, but all delivery history and subscriber records are preserved."` -> User confirms -> [Toast]: `"Newsletter archived"`.
  * **List Presentation:** Newsletter appears with [Status Badge] `"Archived"` (muted gray).
* **Failure / Edge Cases:**
  * *Active Sending In Flight:* If a child broadcast has `status = 'sending'`: [Dialog Modal] alert: `"Cannot archive newsletter while Broadcast 'Release 2.4' is currently sending. Wait for send completion or abort the broadcast first."`

#### UC-C4: Isolation Between Multiple Newsletters
* **Trigger:** Account creates two distinct newsletters: *"Dev Weekly"* and *"Special Offers"*.
* **System Logic:** Database indexes enforce `(newsletter_id, email)` uniqueness. Subscribers in *"Dev Weekly"* do not receive *"Special Offers"* unless explicitly subscribed to both.
* **Presentation:**
  * **UI/UX:** Each newsletter detail view displays isolated metrics: [Status Badge] `"1,420 Active Subscribers"` on Dev Weekly vs `"410 Active Subscribers"` on Special Offers.
...
* **Trigger:** User clicks *"Add Subscriber"* on `/studio/newsletters/:id?tab=subscribers`.
* **System Logic:** Checks email against `studio_account_suppressions` and existing `studio_subscribers` for this newsletter. Generates cryptographically secure `unsubscribe_token`.
* **Success Presentation:**
...
* **Failure / Edge Cases:**
  * *Email Already Subscribed:* [Inline Field] in dialog: `"alex@example.com is already subscribed to this newsletter"`.
...
#### UC-S2: Bulk CSV / Source Import to Newsletter
...
* **Trigger:** Recipient clicks `{{unsubscribe_url}}` rendered as `https://relaybase.email/studio/unsubscribe/:newsletterId/:token`.
* **System Logic:** Look up subscriber by `(newsletterId, unsubscribe_token)`. Sets `status = 'unsubscribed'`, records `unsubscribed_at = now()`.
* **Success Presentation (Public Web View):**
  * **UI/UX:** Dedicated minimal public landing page (`/studio/unsubscribe/confirmed`):
...
* **System Logic:** Sets `Subscriber.status = 'bounced'`, records `bounced_at = now()`, adds email to `studio_account_suppressions`.
* **Presentation:**
...
* **Trigger:** User clicks *"New Broadcast"* inside `/studio/newsletters/:id?tab=broadcasts`.
* **System Logic:** Inserts `studio_broadcasts` row with `status = 'draft'`, sets `subject = ''`, assigns `newsletter.default_template_id`.
* **Success Presentation:**
  * **UI/UX:** Seamless navigation to `/studio/newsletters/:id/broadcasts/:broadcastId/content`. Editor mounts ready for input with [Status Badge] `"Draft"`.

#### UC-B2: Rich Content Autosave
* **Trigger:** User edits subject, BlockNote markdown body, or swaps template wrapper.
* **System Logic:** Debounced autosave (3000ms). Updates `studio_broadcasts` `updated_at`.
* **Success Presentation:**
...
* **System Logic:** Renders current draft with template, substitutes mock tags `{{contact.name}} = "Test Recipient"`, and dispatches via Worker `/v1/send` with `[Test]` prefix. Does not write to `studio_recipients` or affect open/click counters.
* **Success Presentation:**
...
* **System Logic:**
  ...
  2. Resolves `studio_subscribers` for this newsletter `WHERE status = 'subscribed' AND email NOT IN (SELECT email FROM studio_account_suppressions)`.
  3. If count = 0: aborts send.
  4. Flips `studio_broadcasts.status = 'sending'`, sets `sent_at = now()`.
  5. Bulk inserts `studio_recipients` with `status = 'queued'`.
...
* **Failure / Edge Cases:**
  ...
  * *Zero Subscribed Recipients:* [Dialog Modal] alert: `"Cannot send broadcast: This newsletter has 0 active subscribers. Add subscribers before sending."`
...
* **System Logic:** Validates `scheduled_at > now()`. Sets `studio_broadcasts.status = 'scheduled'`. **Does NOT freeze recipients yet.**
* **Success Presentation:**
...
* **System Logic:** API rejects `PATCH /studio/broadcasts/:id` with HTTP 409 Conflict.
* **Presentation:**
...
* **System Logic:** Background Worker loops over `studio_recipients WHERE broadcast_id = :id AND status = 'queued'`:
  ...
  6. Increments `studio_broadcasts.stats_sent` or `stats_failed`.

#### UC-D3: Engagement Tracking (Opens & Clicks)
* **System Logic:**
  * Open Tracking: `GET /studio/t/o/:broadcastId/:recipientId` returns 1x1 transparent GIF; records unique open.
  * Click Tracking: `GET /studio/t/c/:broadcastId/:recipientId?u=:targetUrl` records click timestamp and returns 302 redirect.
* **Presentation:**
  * In `/studio/newsletters/:id/broadcasts/:broadcastId/stats`:
...
[DAY 0 - Monday 10:00]
  1. User creates Newsletter: "Engineering Updates" (slug: "eng-updates")
...
[DAY 0 - Monday 11:00]
  3. User creates Broadcast 1: "v2.4 Release Notes"
     ...
     - NOTE: No rows created in studio_recipients yet!
...
[DAY 1 - Tuesday 07:45] (15 mins prior to Broadcast 1)
  5. bob@ clicks unsubscribe link from a prior newsletter:
     POST /studio/unsubscribe/eng-updates/tok_bob_02
     -> studio_subscribers for bob@ set to: status = 'unsubscribed', unsubscribed_at = 07:45

[DAY 1 - Tuesday 08:00] (Broadcast 1 Dispatch Triggered by Cron)
  6. Cron Worker executes send job for Broadcast 1:
     - Query: SELECT * FROM studio_subscribers WHERE newsletter_id = 'eng-updates' AND status = 'subscribed'
     - Results returned: [ana@, cam@]  (bob@ is automatically excluded!)
     - Inserts 2 studio_recipients rows:
...
[DAY 2 - Wednesday 09:30]
  7. dan@ signs up via webhook or website form:
     POST /studio/newsletters/eng-updates/subscribers { email: "dan@example.com", name: "Dan" }
     -> studio_subscribers creates dan@: status = 'subscribed', token: tok_dan_04
...
[DAY 2 - Wednesday 12:00] (Broadcast 2 Dispatch Triggered by Cron)
  8. Cron Worker executes send job for Broadcast 2:
     ...
     - Inserts 3 studio_recipients rows.
     - Worker dispatches mail sequentially.
...
* **Mitigation:** Unsubscribe URLs utilize a high-entropy 32-byte cryptographically random token (`newId("tok")` / CSPRNG) bound to the `(newsletterId, subscriberId)` tuple. Unsubscribe endpoints require zero authentication but validate token matching.

### 5.3 Global Suppression vs Newsletter Unsubscribe
* **Distinction:**
  * **Newsletter Unsubscribe:** Recipient does not want *"Weekly Tech News"* but continues to receive *"Billing Invoices"* or 1:1 Quote emails. Recorded in `subscribers` with `status = 'unsubscribed'`.
  * **Global Account Suppression:** Recipient issued a Spam Complaint (RFC 8058 / FBL) or generated a Hard Bounce. Recorded in `accountSuppressions`. The dispatch engine unconditionally checks this suppression list on every broadcast dispatch regardless of newsletter membership.
...
```text
/studio
 ├── /pipeline                     (Sales pipeline Kanban — shared Studio entity)
 ├── /quotes                       (Quote composition & e-signature audit)
 └── /newsletters                    (Top-level Newsletter List View)
      ├── /new                     (Create Newsletter Modal Dialog)
      └── /:newsletterId             (Newsletter Shell with Tab Navigation)
...
2. **API Routing Cutover:**
   ...
   * Deprecate global `fetchAllAudienceRecipients()` in frontend code in favor of newsletter-scoped subscriber resolution.
