import type { CampaignDispatchProgress } from "@/lib/scale/api";
...
export function dispatchProcessedCount(dispatch: CampaignDispatchProgress): number {
  return dispatch.queue.processed + dispatch.queue.skipped;
}

export function dispatchRemainingCount(dispatch: CampaignDispatchProgress): number {
  return dispatch.queue.queued + dispatch.queue.inFlight;
}

export function dispatchProgressPercent(dispatch: CampaignDispatchProgress): number {
