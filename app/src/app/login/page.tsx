...
/**
 * Team-user login: a teammate signs in with their account email + the
 * per-account mobile password (same model as the Flutter companion). They
 * never receive the admin token; the desktop talks to the customer Worker's
 * /mobile/* endpoints only.
 */
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!url || !email || !password) {
      setError({
        ...
        detail: "Enter the Worker URL, your account email, and mobile password.",
    ...
    try {
      // Verify the mobile password against the customer Worker.
      ...
      // Email-only mode is rendered by the dashboard when a team login is
      // present and no admin token exists.
      router.replace("/");
    } catch (err) {
      ...
      <div className="space-y-6 rounded-xl border border-border bg-card p-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            password your admin set up in Accounts → Other device. You only
            get inbox access — no management console.
          </p>
        </div>
