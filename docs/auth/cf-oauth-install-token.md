...
| **`~/.relaybase`** | Stores only the resolved CF account id (from the OAuth response) in `workspace.json`. OAuth access/refresh tokens are **never written to disk in plain JSON** — `refresh_token` lives in the OS Keyring and `access_token` lives in Tauri memory only. See **[home-storage.md](../desktop/home-storage.md)**. |
...
| Zone list (Refresh from Cloudflare) | `GET /console/zones` (`../relaybase-worker/src/routes/console/zones.ts`) via `listCloudflareZones` — Worker `CF_API_TOKEN`, not desktop OAuth |
