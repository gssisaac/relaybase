...
// POST /crm/automations/:id/assets { filename, mimeType, contentBase64 }
crmAssets.post("/automations/:id/assets", async (c) => {
  ...
  const url = `${CRM_PUBLIC_BASE_URL}/crm/assets/automation/${encodeURIComponent(automationId)}/${encodeURIComponent(storedFilename)}`;
  return c.json({ url, key });
});

// GET /crm/assets/automation/:automationId/:filename
crmAssets.get("/assets/automation/:automationId/:filename", (c) => {
