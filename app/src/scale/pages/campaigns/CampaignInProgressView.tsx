...
import { scaleApi, type InProgressOverview } from "@/lib/scale/api";
...
export function CampaignInProgressView() {
  ...
  const load = useCallback(async (force?: boolean) => {
    ...
    try {
      setData(await scaleApi.getInProgressOverview());
    } catch {
      toast.error("Could not load in-progress campaigns");
    } finally {
      setRefreshing(false);
    }
  }, []);
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <div className="min-w-0 space-y-2">
          ...
          <CampaignsSectionNav active="in-progress" />
        </div>
      </DesktopTitleBar>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-auto">
        <div className={dashboardScrollBodyClassName("space-y-4")}>
          ...
          {!data ? (
            <p className="text-sm text-muted-foreground">Loading in-progress campaigns…</p>
          ) : (
            <>
              ...
              {data.sending.length === 0 ? (
                <Card>
                  <CardHeader>
                    <CardTitle className="text-sm">No campaigns sending</CardTitle>
                    <CardDescription>
                      Concurrent sends appear here while the queue is draining. Scheduled campaigns
                ...
                <div className="grid gap-3 lg:grid-cols-2">
                  {data.sending.map((row) => (
                      <Card key={row.campaign.id}>
                        <CardHeader>
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              <CardTitle className="truncate text-sm">{row.campaign.name}</CardTitle>
                              <CardDescription className="truncate">
                                {row.campaign.audienceGroupName ?? "No audience"} ·{" "}
                        ...
                        <CardContent className="space-y-3">
                            ...
                            <CampaignSendingProgressPanel
                              ...
                              action={
                                <Button
                                  ...
                                  nativeButton={false}
                                  render={
                                    <Link href={campaignDetailHref(row.campaign.id, "stats")} />
                                  }
                                >
                                  View stats
                                </Button>
                          ...
                          {!row.dispatch ? (
                            <Button
                              ...
                              variant="outline"
                              nativeButton={false}
                              render={<Link href={campaignDetailHref(row.campaign.id, "stats")} />}
                            >
                              View stats
                            </Button>
                          ) : null}
                ...
                {data.scheduled.length === 0 ? (
                  ...
                  <CardContent className="divide-y divide-border p-0">
                    {data.scheduled.map((row) => (
                      <Link
                        key={row.id}
                        href={campaignDetailHref(row.id, "publish")}
