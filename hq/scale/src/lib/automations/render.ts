...
function resolveRelativeAutomationAssetUrl(
  ...
  return `${scaleBaseUrl}/scale/assets/automation/${encodeURIComponent(automationId)}/${encodeURIComponent(filename)}`;
}
...
function sanitizeAutomationContentImages(
  ...
  return html.replace(IMG_TAG_RE, (tag) => {
    ...
    let nextTag = tag;
    const resolved = resolveRelativeAutomationAssetUrl(automationId, scaleBaseUrl, src);
    if (resolved) {
...
export function renderAutomationForSend(input: RenderAutomationInput): string {
  const { automation, automationSendId, templateHtml, scaleBaseUrl } = input;
  ...
  shell = applyTemplateVariablesToHtml(
    ...
    { broadcastId: automationId, scaleBaseUrl },
  );

  const openPixel = `${scaleBaseUrl}/scale/t/a/o/${automationId}/${automationSendId}`;
  ...
  html = html.replace(/href="([^"]*)"/g, (match, url: string) => {
    ...
    const redirect = `${scaleBaseUrl}/scale/t/a/c/${automationId}/${automationSendId}?u=${encodeURIComponent(url)}`;
...
export function buildAutomationListUnsubscribeUrl(
  ...
  return `${scaleBaseUrl}/scale/unsubscribe/automation/${automationId}/${token}`;
}
