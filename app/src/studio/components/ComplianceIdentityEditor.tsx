...
export function ComplianceIdentityEditor({
  ...
  onIdentitySaved?: (identity: StudioComplianceIdentity) => void;
  compact?: boolean;
  description?: string;
}) {
  const [identities, setIdentities] = useState<StudioComplianceIdentity[]>([]);
  ...
  async function saveIdentity() {
    ...
    setSaving(true);
    try {
      const { identity } = await studioApi.updateComplianceIdentity(editingId, {
      ...
      toast.error(err instanceof StudioApiError ? err.message : "Could not save");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function createIdentity() {
    setCreating(true);
    try {
      const { identity } = await studioApi.createComplianceIdentity({
      ...
      if (mode === "account-default") {
        await studioApi.updateAccountLink({ defaultComplianceIdentityId: identity.id });
      }
      onIdentitySaved?.(identity);
      toast.success("Compliance sender created");
    } catch (err) {
      toast.error(err instanceof StudioApiError ? err.message : "Could not create");
    } finally {
      setCreating(false);
    }
  }
