...
export async function GET(req: Request) {
  ...
  if (url.pathname.endsWith("/admin")) {
    if (!(await isAdmin(req, env))) return json({ error: "Unauthorized" }, 401);
    const licenses = await listLicenses(env.KEMBO_LICENSES!);
...
export async function DELETE(req: Request) {
  ...
  if (url.pathname.startsWith("/v1/license/admin/")) {
    ...
    const ok = await revokeLicense(env.KEMBO_LICENSES!, id);
...
async function handleVerify(req: Request, env: CloudflareEnv): Promise<Response> {
  ...
  const record = await verifyLicense(env.KEMBO_LICENSES!, key);
...
async function handleAdminCreate(
  ...
  const { record, licenseKey } = await createLicense(env.KEMBO_LICENSES!, {
    email: body.email,
    tier: body.tier ?? "pro",
