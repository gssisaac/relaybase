...
crm     — contacts, pipeline, newsletters/sequences, quotes, scheduled send (growth/sales)
```
...
**CRM is an intentional exception to that principle.** Original user direction: *“CRM features are not included in the user worker. They are managed on a centrally operated cloud server we run. That central cloud server operates by requesting and querying each user’s worker.”*
