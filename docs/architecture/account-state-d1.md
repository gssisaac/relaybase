...
- **Auth/session tokens** (owner passtoken, access/refresh tokens, mobile passwords) — desktop keeps OS-keyring storage, web keeps in-memory-only sessions. This is a deliberate security trade-off (see [`authentication.md`](../auth/authentication.md)), not something this migration should touch.
