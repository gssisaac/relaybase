...
import { NewsletterSendingProgressPanel } from "@/scale/components/newsletters/NewsletterSendingProgressPanel";
import { NewsletterStatusBadge } from "@/scale/components/newsletters/NewsletterStatusBadge";
import { scaleAudienceDetailHref, newsletterDetailHref } from "@/scale/lib/paths";
...
import { scaleApi, ScaleApiError, type NewsletterDispatchProgress } from "@/lib/scale/api";
...
export function NewsletterPublishView() {
  ...
  async function handleConfirmSend() {
    ...
    try {
      ...
      const result = await scaleApi.sendNewsletter(newsletterId);
  ...
  async function handleTestSend() {
    ...
    try {
      ...
      if (!saved) return;
      if (!(await prepareSendCredentials())) return;
      await scaleApi.testSendNewsletter(newsletterId, testEmail);
  ...
  async function handleSchedule() {
    ...
    try {
      ...
      if (!saved) return;
      if (!(await prepareSendCredentials())) return;
      const updated = await scaleApi.scheduleNewsletter(newsletterId, new Date(scheduleAt).toISOString());
      setNewsletter(updated);
  ...
  async function handleCancelSchedule() {
    try {
      const updated = await scaleApi.cancelSchedule(newsletterId);
      setNewsletter(updated);
                ...
                {newsletter.audienceGroupId ? (
                  <>
                    ...
                    <Button
                      ...
                      nativeButton={false}
                      render={
                        <Link href={scaleAudienceDetailHref(newsletter.audienceGroupId)} />
                      }
                    >
