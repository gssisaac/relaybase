...
import type { PreflightCheck } from "@/studio/lib/newsletters/newsletter-preflight";
...
export function NewsletterPreflightChecklist({
  checks,
  newsletterId,
  settingsHref,
  compact,
  ...
  newsletterId: string;
  /** When omitted, no link to newsletter settings (e.g. message template editor). */
  ...
  return (
    <div className={cn("space-y-2", compact ? "" : "rounded-md border border-border bg-muted/20 p-2.5")}>
      ...
      {(failCount > 0 || warnCount > 0) && !compact && settingsHref !== null ? (
        <Link
          href={settingsHref ?? newsletterDetailHref(newsletterId, "settings")}
