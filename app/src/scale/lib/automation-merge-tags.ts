import type { PreviewRecipient } from "@/scale/lib/broadcast-merge-tags";
import {
  applyBroadcastMergeTags,
  BROADCAST_MERGE_TAGS,
  type ApplyBroadcastMergeTagsOptions,
} from "@/scale/lib/broadcast-merge-tags";
...
export function triggerMergeTagsForAutomation(trigger: AutomationTrigger): ComposeMergeTag[] {
  switch (trigger.type) {
    case "internal_event":
      if (trigger.event === "account.verify_email") {
        return [...INTERNAL_VERIFY_EMAIL_TAGS];
      }
      return [...INTERNAL_CREATED_TAGS];
    case "form_submit": {
      const extra = (trigger.requiredFields ?? []).map(tagFromPayloadPath);
...
export function composeMergeTagSectionsForAutomation(
  ...
  const triggerTags = triggerMergeTagsForAutomation(trigger);
...
export function sampleTriggerPreviewValues(trigger: AutomationTrigger): Record<string, string> {
  const out: Record<string, string> = {};
  for (const tag of triggerMergeTagsForAutomation(trigger)) {
