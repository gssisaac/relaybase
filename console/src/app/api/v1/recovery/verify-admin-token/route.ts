...
export async function POST(req: Request) {
  ...
  if (!env.RELAYBASE_ACCOUNTS) return fail("Recovery not configured", 503);
  ...
  const account = await getAccountById(env.RELAYBASE_ACCOUNTS, accountId);
  ...
  const registered = await env.RELAYBASE_ACCOUNTS.prepare(
