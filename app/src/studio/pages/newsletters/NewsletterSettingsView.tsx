...
export function NewsletterSettingsView() {
  ...
  const [fromName, setFromName] = useState<string | null>(newsletter?.fromName ?? null);
  const [fromEmail, setFromEmail] = useState<string | null>(newsletter?.fromEmail ?? null);
  const [replyTo, setReplyTo] = useState(newsletter?.replyTo ?? "");
  const [defaultLayoutId, setDefaultTemplateId] = useState(newsletter?.defaultLayoutId ?? "");
  ...
  async function saveIdentity() {
    ...
    try {
      ...
      if (err instanceof StudioApiError) setFromEmailError(err.message);
  ...
  async function handleArchive() {
    setArchiving(true);
    try {
      const updated = await studioApi.archiveNewsletter(newsletterId);
      setNewsletter(updated);
      ...
      if (err instanceof StudioApiError) {
        ...
        toast.error("Could not archive newsletter");
      }
    } finally {
      setArchiving(false);
    }
  ...
  async function handleUnarchive() {
    try {
      const updated = await studioApi.unarchiveNewsletter(newsletterId);
      setNewsletter(updated);
      toast.success("Newsletter reactivated");
    } catch {
      toast.error("Could not reactivate newsletter");
    }
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Sender identity and defaults for this newsletter.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Reusable sender records fill the built-in footer at send time. New newsletters use the
        ...
        <CardContent>
          <ComplianceIdentityEditor
            ...
            onSelectedIdentityIdChange={async (id) => {
              if (!id) return;
              try {
                const link = await studioApi.updateAccountLink({
                ...
                toast.error(
                  err instanceof StudioApiError ? err.message : "Could not update default sender",
                );
              }
            }}
          />
      ...
      <Card className="border-destructive/30">
        ...
        <CardFooter className="items-center gap-2">
          {newsletter.listStatus === "archived" ? (
            <>
      ...
      <Dialog open={Boolean(archiveBlocked)} onOpenChange={(open) => !open && setArchiveBlocked(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Cannot archive newsletter</DialogTitle>
