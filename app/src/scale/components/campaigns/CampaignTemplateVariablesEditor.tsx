...
function ImageVariableField({
  ...
  async function onFileSelected(file: File | null) {
    ...
    try {
      ...
      const res =
        assetOwner === "template"
          ? await scaleApi.uploadMessageTemplateAsset(contentOwnerId, {
              filename: file.name,
              mimeType: file.type,
              contentBase64,
            })
          : assetOwner === "trigger"
            ? await scaleApi.uploadTriggerAsset(contentOwnerId, {
                filename: file.name,
                mimeType: file.type,
                contentBase64,
              })
            : await scaleApi.uploadCampaignAsset(contentOwnerId, {
                filename: file.name,
                mimeType: file.type,
                contentBase64,
              });
