...
/**
 * Delivery goes through the customer Worker `POST /v1/send` (crm-mode-v0.2.md §1.2).
 * Set `SCALE_SEND_STUB=1` to log-only (no network) for local UI tests.
 */
...
function useSendStub(): boolean {
  return process.env.SCALE_SEND_STUB === "1" || process.env.SCALE_SEND_STUB === "true";
}
...
export async function sendMail(input: SendMailInput): Promise<SendMailResult> {
  ...
  if (useSendStub()) {
    console.log(
      `[scale-mail-sender:stub] would POST /v1/send → from=${from} to=${to} subject=${JSON.stringify(subject)} (html ${input.html.length} bytes) headers=${JSON.stringify(extraHeaders)}`,
    );
