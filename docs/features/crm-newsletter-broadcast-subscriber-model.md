# Scale Newsletter, Broadcast & Subscriber Architecture Specification

> **Historical draft — superseded (2026-09-15).**  
> The shipped Scale model **removed the Newsletter / Subscriber entities** in favor of **Audience Group → Broadcast**.  
...
1. **Newsletter = Consent Scope**: Unsubscribing from *"Product Updates"* flips `Subscriber.status = 'unsubscribed'` on that newsletter only. The user remains `subscribed` in *"Security Advisories"* and retains their Scale Lead/Customer Pipeline status.
...
* **Trigger:** User clicks *"New Newsletter"* in the `/scale/newsletters` header toolbar.
...
* **Success Presentation:**
  ...
  * **View Transition:** User is redirected directly to `/scale/newsletters/:id?tab=subscribers`.
* **Failure / Edge Cases:**
...
* **Trigger:** User modifies default From Name, From Email, or Default Layout Template in `/scale/newsletters/:id?tab=settings`.
* **System Logic:** Validates email format, saves to `crm_newsletters`. Future broadcasts inherit these defaults automatically.
* **Success Presentation:**
...
* **Trigger:** User clicks *"Add Subscriber"* on `/scale/newsletters/:id?tab=subscribers`.
* **System Logic:** Checks email against `crm_account_suppressions` and existing `crm_subscribers` for this newsletter. Generates cryptographically secure `unsubscribe_token`.
* **Success Presentation:**
...
* **Trigger:** Recipient clicks `{{unsubscribe_url}}` rendered as `https://crm.relaybase.xyz/scale/unsubscribe/:newsletterId/:token`.
...
* **Trigger:** User clicks *"New Broadcast"* inside `/scale/newsletters/:id?tab=broadcasts`.
* **System Logic:** Inserts `crm_broadcasts` row with `status = 'draft'`, sets `subject = ''`, assigns `newsletter.default_template_id`.
* **Success Presentation:**
  * **UI/UX:** Seamless navigation to `/scale/newsletters/:id/broadcasts/:broadcastId/content`. Editor mounts ready for input with [Status Badge] `"Draft"`.
...
* **System Logic:**
  ...
  2. Resolves `crm_subscribers` for this newsletter `WHERE status = 'subscribed' AND email NOT IN (SELECT email FROM crm_account_suppressions)`.
...
* **Presentation:**
  * In `/scale/newsletters/:id/broadcasts/:broadcastId/stats`:
...
[DAY 1 - Tuesday 08:00] (Broadcast 1 Dispatch Triggered by Cron)
  6. Cron Worker executes send job for Broadcast 1:
     - Query: SELECT * FROM crm_subscribers WHERE newsletter_id = 'eng-updates' AND status = 'subscribed'
...
[DAY 2 - Wednesday 09:30]
  7. dan@ signs up via webhook or website form:
     POST /scale/newsletters/eng-updates/subscribers { email: "dan@example.com", name: "Dan" }
...
2. **API Routing Cutover:**
   * `/scale/newsletters` endpoints updated to serve the Newsletter -> Broadcast hierarchy.
