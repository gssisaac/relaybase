# Scale Compliance & Platform Improvements Backlog

**Status:** Active engineering checklist  
**Scope:** `hq/scale` + `app/src/crm` (Audience Group → Broadcast model)  
**Date:** 2026-09-15  
...
| Risk | Mitigation |
|------|------------|
| AUP / spam complaint rate on customer Worker or Scale domain | Suppression ledger, rate-limited dispatch, no send without auth |
| Worker subrequest / CPU limits on bulk send | Queue + batch dispatch (scheduler), not single-request fan-out |
| Open/click tracking write spikes on Scale store | Buffered/batched tracking writes in production (same tenant as `hq/scale`) |
| Open redirect abuse on `/scale/t/c` | Allow only `http:` / `https:` targets |
| Trust & Safety phishing via Scale domain | Same redirect guard + monitoring |
...
| 3 | **Click redirect guard** (`/scale/t/c`) — block non-http(s) schemes | Done |
...
Production Scale is intended to run on **the customer’s own Cloudflare stack** (`hq/scale` beside their Worker), not a central Relaybase D1 catalog. Persistence may evolve from dev JSON to a tenant-local store; **no separate “D1 migration” track** is planned here.
...
| 12 | Webhook HMAC for `/scale/webhooks/bounce` |
