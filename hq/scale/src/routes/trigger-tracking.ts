...
export const scaleTriggerTracking = new Hono();

scaleTriggerTracking.get("/o/:triggerId/:triggerSendId", async (c) => {
...
scaleTriggerTracking.get("/c/:triggerId/:triggerSendId", async (c) => {
  const { triggerId, triggerSendId } = c.req.param();
