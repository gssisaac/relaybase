...
See the full breakdown of what each address is for in [standard product email addresses, explained](/resources/standard-product-email-addresses), or the mechanics of how send-and-receive works without a mailbox in [transactional email: send and receive on one domain](/resources/transactional-email-send-and-receive). Ready to see the flat-rate version in your own stack? [Join the Relaybase beta](/get-started).
