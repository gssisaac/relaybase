...
| Layer | Store | Use for |
|-------|--------|---------|
| Remote | D1 `RELAYBASE_DB` (`server/db/app/`, binding `RELAYBASE_DB`) | All durable product state: domains, addresses, audience groups/contacts, broadcasts, branding, API keys, dashboard auth tokens, mobile passwords, webhooks, owner config (including recovered `admin_token`), `app_settings` (inbound retain-per-domain; default unlimited), pending inbound events. Sole source of truth — no KV. |
