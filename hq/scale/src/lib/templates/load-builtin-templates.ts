...
export function resolveBuiltinTemplatesDir(): string {
  if (process.env.SCALE_BUILTIN_TEMPLATES_DIR) {
    return path.resolve(process.env.SCALE_BUILTIN_TEMPLATES_DIR);
  }
...
export function loadBuiltinTemplates(): BuiltinTemplate[] {
  ...
  if (!fs.existsSync(root)) {
    console.warn(`[scale] built-in templates dir missing: ${root}`);
    return [];
  }
