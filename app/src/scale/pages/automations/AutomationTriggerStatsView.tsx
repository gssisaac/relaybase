...
import { scaleApi, type AutomationTriggerStatsOverview } from "@/lib/scale/api";
...
export function AutomationTriggerStatsView() {
  const [data, setData] = useState<AutomationTriggerStatsOverview | null>(null);
  ...
  const load = useCallback(async (force?: boolean) => {
    ...
    try {
      setData(await scaleApi.getAutomationTriggerStats());
    } catch {
      ...
      >
        <div className="min-w-0 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Triggers, delivery, and engagement across all automations.
          </p>
        </div>
      </DesktopTitleBar>
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {data.totals.automations} automations · sorted by lifetime triggers
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                  {data.byAutomation.length === 0 ? (
                    ...
                    <ul className="divide-y divide-border">
                      {data.byAutomation.map((row) => (
                        <li key={row.id}>
                          <Link
                            href={automationDetailHref(row.id, "stats", row.status)}
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Latest fires across all automations</CardDescription>
                </CardHeader>
                <CardContent className="p-0">
