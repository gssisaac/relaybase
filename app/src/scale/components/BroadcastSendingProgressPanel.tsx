...
import {
  ...
  formatInMinutes,
  formatWhen,
} from "@/scale/lib/broadcast-dispatch-display";
import type { BroadcastDispatchProgress } from "@/lib/scale/api";

type Props = {
  dispatch: BroadcastDispatchProgress;
  /** Optional action (e.g. link to stats) aligned to the right on wide screens. */
  action?: ReactNode;
  compact?: boolean;
};

export function BroadcastSendingProgressPanel({ dispatch, action, compact }: Props) {
