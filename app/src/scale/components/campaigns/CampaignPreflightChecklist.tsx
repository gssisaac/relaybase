...
import type { PreflightCheck } from "@/scale/lib/campaigns/campaign-preflight";
...
export function CampaignPreflightChecklist({
  checks,
  campaignId,
  settingsHref,
  compact,
  ...
  campaignId: string;
  /** When omitted, no link to campaign settings (e.g. message template editor). */
  ...
  return (
    <div className={cn("space-y-2", compact ? "" : "rounded-md border border-border bg-muted/20 p-2.5")}>
      ...
      {(failCount > 0 || warnCount > 0) && !compact && settingsHref !== null ? (
        <Link
          href={settingsHref ?? campaignDetailHref(campaignId, "settings")}
