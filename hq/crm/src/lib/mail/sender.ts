 ...
 * Set `CRM_SEND_STUB=1` to log-only (no network) for local UI tests.
 */
...
function useSendStub(): boolean {
  return process.env.CRM_SEND_STUB === "1" || process.env.CRM_SEND_STUB === "true";
}
...
export async function sendMail(input: SendMailInput): Promise<SendMailResult> {
  ...
  if (useSendStub()) {
    console.log(
      `[crm-mail-sender:stub] would POST /v1/send → from=${from} to=${to} subject=${JSON.stringify(subject)} (html ${input.html.length} bytes) headers=${JSON.stringify(extraHeaders)}`,
    );
