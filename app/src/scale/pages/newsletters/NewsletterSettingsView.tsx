...
export function NewsletterSettingsView() {
  ...
  async function handleArchive() {
    setArchiving(true);
    try {
      const updated = await scaleApi.archiveNewsletter(newsletterId);
      setNewsletter(updated);
  ...
  async function handleUnarchive() {
    try {
      const updated = await scaleApi.unarchiveNewsletter(newsletterId);
      setNewsletter(updated);
        ...
        <CardContent className="flex flex-wrap items-center justify-between gap-3">
          ...
          {newsletter.audienceGroupId ? (
            <Button
              size="sm"
              variant="outline"
              render={<Link href={scaleAudienceDetailHref(newsletter.audienceGroupId)} />}
            >
              Open subscribers
            </Button>
          ) : null}
