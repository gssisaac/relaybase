...
export const scaleAutomationTracking = new Hono();

scaleAutomationTracking.get("/o/:automationId/:automationSendId", async (c) => {
...
scaleAutomationTracking.get("/c/:automationId/:automationSendId", async (c) => {
  const { automationId, automationSendId } = c.req.param();
