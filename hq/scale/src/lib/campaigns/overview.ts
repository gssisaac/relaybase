...
import type { Campaign, CampaignStats, Recipient, TrackingEvent } from "../../db/types";
import { requireMessage } from "../messages/resolve";
import { buildCampaignDispatchProgress, type CampaignDispatchProgress } from "./dispatch-progress";
import { emptyCampaignStats } from "./stats";

function resolveCampaignSubject(row: Campaign): string {
...
export type AccountCampaignSentOverview = {
...
export type CampaignInProgressOverview = {
  sending: Array<{
...
export function buildSentOverview(input: {
...
}): AccountCampaignSentOverview {
  const sentRows = input.campaigns
    .filter((b) => b.status === "sent" || b.status === "failed")
    .sort((a, b) => (b.sentAt ?? b.createdAt).localeCompare(a.sentAt ?? a.createdAt));

  const totals = emptyCampaignStats();
  ...
  const linkMap = new Map<
    string,
    { url: string; clicks: number; unique: Set<string>; campaignId: string }
  >();
  for (const event of input.trackingEvents) {
    if (!sentIds.has(event.campaignId) || event.type !== "click" || !event.url) continue;
    const key = `${event.campaignId}::${event.url}`;
    let row = linkMap.get(key);
    if (!row) {
      row = { url: event.url, clicks: 0, unique: new Set(), campaignId: event.campaignId };
      linkMap.set(key, row);
    }
    row.clicks += 1;
  ...
  return {
    period,
    totals: { ...totals, campaigns: campaignsCount },
    rates: {
    ...
    campaigns: sentRows.map((row) => ({
      id: row.id,
      name: row.name,
      subject: resolveCampaignSubject(row),
      status: row.status,
...
export function buildCampaignInProgressOverview(input: {
  sending: SerializedCampaign[];
  scheduled: SerializedCampaign[];
  recipients: Recipient[];
  trackingEvents: TrackingEvent[];
}): CampaignInProgressOverview {
  return {
    sending: input.sending.map((broadcast) => {
      const recips = input.recipients.filter((r) => r.campaignId === broadcast.id);
      ...
      const recentEvents = input.trackingEvents
        .filter((e) => e.campaignId === broadcast.id)
      ...
      return {
        ...
        lastDispatchedAt,
        dispatch: buildCampaignDispatchProgress({
          recipients: recips,
          startedAt: broadcast.startedAt,
        }),
        recentEvents,
