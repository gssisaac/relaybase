...
import { crmApi, CrmApiError, type AutomationTrigger } from "@/lib/crm/api";
...
export function AutomationTriggerView() {
  ...
  async function saveTrigger() {
    ...
    try {
      ...
      let updated = await crmApi.updateAutomation(automationId, {
        trigger: patch,
        cooldownSeconds,
        applyMarketingSuppression: applySuppression,
      });
      if (patch.type === "http_webhook" && patch.secret === "pending-rotate") {
        const rotated = await crmApi.rotateAutomationWebhookSecret(automationId);
      ...
      toast.error(e instanceof CrmApiError ? e.message : "Could not save trigger");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleActivate() {
    ...
    try {
      await saveTrigger();
      const updated = await crmApi.activateAutomation(automationId);
      setAutomation(updated);
      toast.success("Automation is active");
    } catch (e) {
      const issues =
        e instanceof CrmApiError && Array.isArray(e.body?.issues)
          ? (e.body!.issues as { message: string }[])
          : null;
      toast.error(
        issues?.map((i) => i.message).join(" · ") ??
          (e instanceof CrmApiError ? e.message : "Could not activate"),
      );
    } finally {
      setActivating(false);
    }
  ...
  async function handlePause() {
    try {
      const updated = await crmApi.pauseAutomation(automationId);
      setAutomation(updated);
  ...
  async function rotateSecret() {
    try {
      const res = await crmApi.rotateAutomationWebhookSecret(automationId);
  ...
  async function sendTest() {
    ...
    setTestSending(true);
    try {
      await crmApi.testSendAutomation(automationId, { email, name: "Test User" });
      toast.success("Test email sent");
      setTestOpen(false);
      await refresh();
    } catch (e) {
      toast.error(e instanceof CrmApiError ? e.message : "Test send failed");
    } finally {
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                <code className="rounded bg-muted px-1">
                  POST /crm/automations/fire/internal/{internalEvent}
                </code>{" "}
                (CRM API auth).
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              {formHookUrl ? (
                <div className="space-y-1 sm:col-span-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Uses the same CRM webhook secret as bounce webhooks.
                  </p>
                </div>
              ) : null}
            </div>
          ...
          {triggerType === "mailbox_inbound" ? (
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <p className="text-xs text-muted-foreground sm:col-span-2">
                Worker posts to{" "}
                <code className="rounded bg-muted px-1">POST /crm/hooks/inbound</code> with domain,
