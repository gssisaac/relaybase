...
const CRM_SEND_KEY_LABEL = "CRM Broadcast";
...
/**
 * Ensure hq/crm has a domain-scoped Worker API key before dispatch.
 * Issues or reuses a key labeled "CRM Broadcast" and PATCHes account-link.
 */
export async function syncCrmSendCredentials(input: {
  apiBase: string;
  ...
  if (!match?.apiKey) {
    const createRes = await desktopAwareFetch(`${input.apiBase}/keys`, {
      ...
      body: JSON.stringify({ domain, label: CRM_SEND_KEY_LABEL }),
    });
    ...
    if (!createRes.ok || !created.apiKey || !created.id) {
      throw new Error(created.error ?? "Could not issue CRM send API key");
    }
    ...
    match = {
      id: created.id,
      domain: created.domain ?? domain,
      label: created.label ?? CRM_SEND_KEY_LABEL,
      active: true,
      apiKey: created.apiKey,
    };
  }
