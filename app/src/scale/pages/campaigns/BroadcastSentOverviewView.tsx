...
import { scaleApi, type AccountSentOverview } from "@/lib/scale/api";
...
export function BroadcastSentOverviewView() {
  ...
  const load = useCallback(async (force?: boolean) => {
    ...
    try {
      setData(await scaleApi.getSentOverview());
    } catch {
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <div className="min-w-0 space-y-2">
          ...
          <BroadcastsSectionNav active="sent" />
        </div>
      </DesktopTitleBar>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-auto">
        <div className={dashboardScrollBodyClassName("space-y-4")}>
          ...
          {!data ? (
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">No sent broadcasts yet</CardTitle>
                <CardDescription>
            ...
            <>
              <p className="text-xs text-muted-foreground">
                ...
                {data.totals.broadcasts} broadcasts
              </p>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Finished sends, newest first. Open a row for per-broadcast stats.</CardDescription>
                </CardHeader>
                <CardContent className="divide-y divide-border p-0">
                  {data.broadcasts.map((row) => (
                    <Link
                      key={row.id}
                      href={broadcastDetailHref(row.id, "stats")}
              ...
              {data.topLinks.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Tracked clicks across finished broadcasts.</CardDescription>
                  </CardHeader>
                  <CardContent className="divide-y divide-border p-0">
                    {data.topLinks.map((row) => (
                      <div
                        key={`${row.broadcastId}:${row.url}`}
                        className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm"
                      >
                        <a
                          href={row.url}
